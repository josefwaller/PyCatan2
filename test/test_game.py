...
def test_can_get_longest_road():
    ...
    g.players[0].add_resources(get_resource_hand(lumber=5, brick=5))
    coords = (
        Coords(1, 0),
        Coords(0, 1),
        Coords(0, 2),
    ...
    build_road_along_path(g, g.players[0], coords[:5])
    assert g.longest_road_owner is None
    build_road_along_path(g, g.players[0], coords[4:6])
...
def test_can_steal_longest_road():
    ...
    g.players[0].add_resources(get_resource_hand(lumber=5, brick=5))
    g.players[1].add_resources(get_resource_hand(lumber=6, brick=6))
